...
{
    // The minimum is roughly one item wide — NOT the sum of every item — which
    // is what frees the host panel to shrink and lets the row wrap.
    QSize size;
...
int FlowLayout::doLayout(const QRect &rect, bool testOnly) const
{
    ...
    for (QLayoutItem *item : m_items) {
        ...
        if (wid && !wid->isVisibleTo(parentWidget()))
            continue; // a hidden item must not reserve a slot or a blank gap
