...
| **25 §1 — kimi fork** | kimi implements fork internally and the spike reached it over `kimi web` — **but the owner rejected that route on authority grounds** (the server exposes full session control, `/terminals/*` and `/files/*` behind a same-uid-readable token). Mechanism rejected; the specification it revealed is kept. | `ExportSession` ships first — independent, plain CLI. Then a probe of what kimi's own fork writes **on disk**, to replicate it without a server. `Capabilities.Fork` stays false for kimi until that lands; if it cannot be done faithfully, the honest outcome is the summary-seeded flavour labelled as such, not a lossy thing called "fork". |
